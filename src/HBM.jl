    ...
    elseif m % s == 0
        ...
        if V == 3
            ...
            if length(Xsolutions) == 0
                if output > 0
                    printf("No X solutions", line = true)
                    printEnd()
                end
                return 1 // 1
            elseif a == (2 * d)
                ...
                if output > 0
                    printf(
                        "Since a=2d, hbm($m, $s) = fc($m, $s). Therefore, upperbound of hbm($m, $s) is $alpha.",
                        line = true,
                    )
                    printEnd()
                end
                ...
                if vhbm(a, d, k, X, output = output)
                    if output > 0
                        printfT(
                            "Deriving alpha",
                            "The derived value for X is $X. f($m, $s) ≤ (dk+X)/(3dk+a).",
                            "",
                    ...
                    if output > 0
                        printf(
                            "VHBM does not verify the alpha derived by Hard-Buddy Match.",
                            line = true,
                        )
                        printEnd()
                    end
            ...
            if output > 0
                printf(
                    "Hard-buddy match method requires that students are given 3-shares and 2-shares.
...
function vhbm(a, d, k, X; output::Int64 = 2)
    ...
    if a == (2 * d) #use FC
        alpha = fc(muffins, students, output = output)
        if output > 1
    ...
    if pre && V == 3
        ...
        if output > 1 #start of proof
            ...
            printf(
                "Assume, by way of contradiction, there is a ($muffins, $students) procedure where the smallest piece is > $alphaS.",
                "",
            ...
            printfT(
                "Solving for students and shares",
                "Every student has $(V) or $(V-1) shares.",
                "",
                "Let s₂ and s₃ equal the number of 2 and 3-students.",
                "",
            ...
            printfT(
                "Variables",
                "We will use the following variables for the HBM algorithm (where m=muffins and s=students):",
                "",
                "d = m-s = $d",
                "",
            ...
            printfT(
                "Note",
                "A buddy of share size x is equal to 1-x. A match of share size y is equal to $muffins/$students-y.",
            )
        end
        ...
        bij1, bij2 = formatFrac((2 * d * k + a - d - X) // comden, den),
        formatFrac((2 * d * k - d + 2 * X) // comden)
        int1, int2 = formatFrac(((d * k + a - X) // comden), den),
        formatFrac(((d * k + 2X) // comden), den)
        int3, int4 = formatFrac((2 * d * k + a - 2 * X) // comden, den),
        ...
        if output > 1
            ...
            printfT(
                ...
                "B₀ = B(M₀) = [$bij1, $bij2]",
                "",
                "∀ ≤ i ≤ $(k-1), [Mᵢ = M(Bᵢ₋₁) = [($(d*k+a)+$d(i+1)-2X)/$comden, ($(d*k)+$(d)(i+1)+X)/$comden)]",
                "",
                "∀ ≤ i ≤ $(k-1), [Bᵢ = B(Mᵢ) = [($(2*d*k+a-X)-$(d)(i+1))/$comden, ($(2*d*k+2*X)-$(d)(i+1))/$comden)]",
            ) #printing bijections

            printfT(
                "Note",
                "Since M₀ is empty, so is the Mᵢ's' and Bᵢ's.",
            ...
            printfT(
                "HBM Algorithm",
                "Algorithm proves through buddy-match bijections that 𝑧 = 2a+2d = $(z).",
                "",
            ...
            if k % 2 != 0
                casekeyS, casenum, endsum =
                    formatFrac((k + 1) // 2), 2, (3 * d * k + a + d) // comden
                midpointS = formatFrac(endsum * 1 // 2)
            ...
            printfT(
                "Case $casenum",
                "k ($k) is $parity. Let i = $ivalue = $casekeyS.",
                "",
                "Mᵢ = [$sym1, $sym2].",
                "",
        ...
        if len > 0 && (2, 0, 1) in workingcomb
            if output > 1
                sharecombs = [
                    "$a shares from J1, $b shares in J2, and $c shares in J3"
                    for (a, b, c) in workingcomb
                ]
                printHeader("POSSIBLE SHARES: ")
                printfT(
                    "Determining share combos",
                    "Given all possible share combos, the only shares from J1, J2, and J3 that are not forbidden are: ",
                    "",
                    sharecombs...,
                    "",
                )
            ...
            if len == 1
                if output > 1
                    printfT(
                        ...
                        "Since there is only 1 possible share combination, we look for a 2d solution in following set of equations:",
                        "",
                        "Equation 1 based on |J₁|+ |J₂| = a+d → 2y₁₁₃ = a+d = $(a+d)",
                        "",
                        "Equation 2 based on |J₃| = 4d-2a → y₁₁₃ = 4d-2a = $(4*d-2*a)",
                        "",
                        "Equation 3 based on s₃ = 2d → y₁₁₃ = 2d = $(2*d)",
                        "",
...
function combs(targ::Any, n::Any)
    ...
    if n == 0 #if length is 0, return empty vector
        ...
        splitting(x, n) = [x[i:min(i + n - 1, length(x))] for i = 1:n:length(x)] #function will split targperm output into increments of length n

        append!(finalsol, splitting(othersolutions, n))
    end
end

# Helper function for combs -- returns permuations of target number and 0
