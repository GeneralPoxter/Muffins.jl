...
function int(m::Int64, s::Int64; output::Int64 = 2)
    ...
    elseif m % s == 0
        if output > 0
            printfT("Interval Method", "Since $m % $s = 0, muffins($m,$s) = 1")
            printEnd()
        end
        return 1 // 1
    end
    ...
    if numW > numV
        ...
        alpha = min(
            ((W - f)W - (W - f + 1)m // s + f) // ((W - f - 1)W + 2f),  # Value for alpha derived by solving f(1-α) + (W-f)(1-y) = m/s
    ...
    elseif numW < numV
        ...
        alpha = min(
            ((V - f)W + (2f - V - 1)m // s) // ((V - f)W + (f - 1)V),   # Value for alpha derived by solving f(1-x) + (V-f)(1-y) = m/s
...
function vint(m::Int64, s::Int64, alpha::Rational{Int64}; output::Int64 = 2)
    ...
    elseif alpha > 1
        ...
        if x > y || x == alpha || y == 1 - alpha
            output == 1 &&
                printf("Could not generate disjoint intervals, VInt failed", line = true)
            return false
        end
        ...
        elseif lowerB >= m // s
            ...
            if output > 1
                ...
                printf(
                    "Bounding # of large and small $vMax-shs inconclusive, restarting with individual case analysis",
                    line = true,
                )
            end
        ...
        if output > 1
            ...
            printEnd()
        elseif output > 0
            printfT("Interval Method", "Upper bound α of muffins($m,$s) is $alpha")
            printEnd()
        end
