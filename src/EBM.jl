...
function ebm(m::Int64, s::Int64; output::Int64 = 2)
    ...
    elseif m % s == 0
        if output > 0
            printfT("Easy Buddy Match", "Since $m % $s = 0, muffins($m,$s) = 1")
            printEnd()
        end
        return 1 // 1
    end
...
function ebmproof(m, s, alpha)
    ...
    if V == 3 #ebm only works if there are 2-shares
        ...
        printfT(
            "Note",
            "Let the common denominator $den. Therefore, alpha will be referred to as $alphaden.",
        )

        printHeader("DEFINING TERMS FOR EBM ALGORITHM: ")
        printfT(
            "Variables",
            "We will use the following variables for the EBM algorithm (where m=muffins and s=students):",
            "",
            "d = m-s = $d",
            "",
            "k is the largest number >= 0 such that 3dk > s. k = $k.",
            "",
            "a = s - 3dk = $a",
        )
        printfT(
            "Common Terms",
            "A buddy of share size x is equal to 1-x. A match of share size y is equal to $m/$s-y.",
        )
        ...
        if a == 2 * d
            ...
            printfT(
                "Important note",
                "The proof is conducted using parameter X. Assume that there is a ($m, $s) procedure where X ≥ $(firstparameter) so alpha ≥ $firstparameter.",
                "",
            ...
            if case1 * (1 // (V + 1)) < alpha
                printfT(
                    "Case 1",
                    "If Alice gets ≥ $(V+1) shares then some share is ≤ ",
                    "",
                    "$case1str * $V_case1 ≤ $v_conj1, which is < $alphaden",
                    "",
            ...
            if (V - 2) == 1 && (3 * d * k + a + d) // (comden) > 1 // 2
                printfT(
                    "Case 2",
                    "If Alice gets $(V-2) share, since each share is ≤ 1/2, she has:",
                    "",
            ...
            printfT(
                "Case 3",
                "Everyone is a $V or $(V-1) student. Let s_2 & s_3 be the number of 2-students and 3-students. Since there are $(6*d*k + 2*a +2*d) shares: ",
                "",
                "",
                "$(V)s_$V + $(V-1)s_$(V-1) = $(6*d*k + 2*a +2*d)",
                "",
                "s_$V + s_$(V-1) = $(3*d*k +a)",
                "",
            ...
            printHeader("CONTINUING CASEWORK")
            printfT(
                "Case 3a",
                "There is a $V-share ≥ ($(d*k+a+d)-2X)/$comden. The remaining $(V-1) $V-shares add up to ",
                "",
                "≤ $m/$s - ($(d*k+a+d)-2X)/$comden = ($(2*d*k)+2X)/$comden.",
                "",
                "Hence there is a $V-share ($(2*d*k)+2X)/$comden * 1/2 = ($(d*k)+X)/$comden",
            )
            printfT(
                "Case 3b",
                "There is a $(V-1) share ≥ ($(2*d*k+a)-X)/$comden. Its buddy is ",
                "",
                "≤ 1 - ($(2*d*k+a)-X)/$comden  = ($(d*k)+X)/$comden",
            )
            printfT(
                "Case 3c",
                "There is a $(V-1) share ≤ ($(d*k+d)+X)/$comden. Its match is ",
                "",
                "≥ $total - ($(d*k+d)+X)/$comden  = ($(2*d*k+a)-X)/$comden",
                "",
                "Its buddy is ≤ 1 - ($(2*d*k+a)-X)/$comden  = ($(d*k)+X)/$comden",
            )
            printfT(
                "Case 3d",
                "There is a $V-share ≤ ($(d*k)+X)/$comden. This case is obvious.",
            )
            printfT(
                "Case 3e",
                "All $V-shares are in (($(d*k)+X)/$comden, ($(d*k+a+d)-2X)/$comden))",
                "",
            ...
            j = k - 1
            #Define bijections
            printHeader("BUDDY/MATCH ANALYSIS:")

            printfT(
                "Buddy/Match Bijections",
                "We define the following (where B signifies buddying and M signifies matching):",
                "",
                "M₀ = (($(d*k)+X)/$comden, ($(d*k+a+d)-2X)/$comden))",
                "B₀ = B(M₀) = (($(2*d*k+a-d)-X)/$comden), ($(2*d*k+a)-X)/$comden)",
                "",
                "∀ ≤ i ≤ $(k-1), [Mᵢ = M(Bᵢ₋₁) = (($(d*k)+$(d)i+X)/$comden, $(d*k)+$(d)(i+1)+X)/$comden)]",
                "",
            ...
            printfT(
                "Important note",
                "The last B (M) is B𝒌 (M𝒌₋₁)).",
                "",
                "Since we might have an interval that overlaps the 3-share region, and can't apply M to it, we stop here.",
                "",
                "If k ≥ 2, B𝒌₋₂ is in the same $(V-1)-share region and M() can be applied to it. This is good since M𝒌₋₁ needs B𝒌₋₂ to be in the $(V-1)-shr region.",
            ) #defining restrictions


            printfT(
                "Translating into endpoints",
                "Therefore, we want the left endpoint of B𝒌₋₂ ≥ the right endpoint of the $V-shares.",
                "",
                "($(d*k+a+d)-2X)/$comden) ≤ ($(2*d*k+a-(k-1)*d)-X)/$comden",
                "This is equal to 1 ≤ 2, a true statement.",
            )
            printfT(
                "Final note",
                "|B₀| = $(6*d). Since B and M are bijections: ",
                "",
                "1. every Bᵢ and Mᵢ have $(6*d) shares",
                "",
                "2. if x is an endpoint of Bᵢ or Mᵢ, there are no shares size x",
            ) #final point
            ...
            if 2 * d + 1 <= a <= 3 * d #where X=a/3
                printfT(
                    ...
                    "",
                    "This is X<d, which is true since X=$(a)/3 and a ≤ 3d which equals $a ≤ $(3*d)",
                    "",
                    "B₀∪...∪B𝒌₋₁ ⊇ (($(d*k+d)+X)/$comden, ($(2*d*k+a)-X)/$comden)",
                    "",
                    "(Right hand set is $(V-1) shares)",
                    "",
                    "|B₀∪...∪B𝒌₋₁| ≥(($(d*k+d)+X)/$comden, ($(2*d*k+a)-X)/$comden)",
                    "",
                    "6*d*k ≥ 6*d*k + 2*a - 4*d",
                    "",
                    "Simplifying: $(2*d)≥$a. This is our contradiction.",
                )
            ...
            elseif 1 <= a <= d #where X=a/2
                printfT(
                    ...
                    "",
                    "Since X=$a/2 is true by assumption",
                    "",
                    "B₀∪...∪B𝒌₋₁ ⊇ (($(d*k)+X)/$comden, ($(2*d*k+a)-X)/$comden)",
                    "",
                    "(Right hand set is $(V-1) shares)",
                    "",
                    "|B₀∪...∪B𝒌₋₁| ≥(($(d*k)+X)/$comden, ($(2*d*k+a)-X)/$comden)",
                    ...
                    "6*d*k ≥ 6*d*k + 2*a + 2*d",
                    "",
                    "Simplifying: a+d=0 or $a+$d=0. This is our contradiction, since d≥1.",
                )
            ...
            elseif d <= a <= 2 * d - 1 #where X=(a+d)/4 this is unfinished - have not tested yet.
                printfT(
                    ...
                    "B₀ = B(M₀) = (($(2*d*k+a-d)-X)/$comden), ($(2*d*k+a)-X)/$comden)",
                    "",
                    "∀ ≤ i ≤ $(k), [Mᵢ = M(Bᵢ₋₁) = (($(d*k)+$(d)i+X)/$comden, $(d*k)+$(d)(i+1)+X)/$comden)]",
                    "",
                    "∀ ≤ i ≤ $(k), [Bᵢ = B(Mᵢ) = (($(2*d*k+a)-$(d)(i-1)-X)/$comden, ($(2*d*k+a)-$(d)(i)-X)/$comden)]",
                    "",
                    "Note: B𝒌₋₁ = (($(d*k+a)-X)/$comden, ($(d*k+a+d)-X)/$comden",
                    "",
                    "We need ($(d*k+a)-X)/$comden ≤ ($(d*k+a+d)-X. This is true.",
                    "",
            ...
            if contra
                ...
                printfT(
                    "Conclude alpha",
                    "Each possible case verifies a lower bound for α that contradicts the assumption",
                    "",
